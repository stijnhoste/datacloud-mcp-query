...
**Last tested:** 2026-01-07 on `mca-next-sdo` org

| Status | Count | Description |
|--------|-------|-------------|
| ✅ Verified Working | 66 | Tools tested and confirmed working |
| ❌ Issues | 88 | See details below |
| **Total** | **154** | (2 tools are aliases) |

### ✅ Verified Working Tools (66)

These tools have been tested and work correctly:

**Org & Query:**
- `list_orgs`, `set_target_org`, `get_target_org`
- `query`, `list_tables`, `describe_table`, `validate_query`, `format_sql`
- `cancel_sql_query`, `query_v2`, `get_query_batch_v2`

**Metadata & Schema:**
- `get_metadata`, `describe_table_full`, `get_relationships`, `explore_table`

**Profile:**
- `get_profile_record`

**Segments:**
- `list_segments`, `get_segment`, `get_segment_members`

**Activations:**
- `list_activations`, `get_activation`, `list_activation_targets`, `get_activation_target`

**Data Streams:**
- `list_data_streams`, `get_data_stream`

**Data Transforms:**
- `list_data_transforms`, `get_data_transform`, `get_transform_run_history`, `get_transform_schedule`, `cancel_data_transform`

**Connections:**
- `list_connectors`, `list_connections`, `get_connection`, `get_connection_objects`

**DLOs & DMOs:**
- `list_data_lake_objects`, `get_data_lake_object`, `create_data_lake_object`, `update_data_lake_object`
- `list_data_model_objects`, `get_data_model_object`, `get_dmo_relationships`

**Data Spaces:**
- `list_data_spaces`, `get_data_space`, `get_data_space_members`, `get_data_space_member`

**Calculated Insights:**
- `list_calculated_insights`, `get_calculated_insight`, `get_insight_metadata`, `query_calculated_insight`, `run_calculated_insight`

**Data Graphs:**
- `list_data_graphs`, `get_data_graph`

**Identity:**
- `list_identity_rulesets`, `get_identity_ruleset`

**ML & AI:**
- `list_ml_models`, `get_ml_model`, `list_model_artifacts`, `get_model_artifact`
- `list_document_ai_configs`, `get_document_ai_global_config`
- `list_semantic_searches`, `get_semantic_search_config`

**Data Actions:**
- `list_data_actions`, `list_data_action_targets`, `get_data_action_target`

**Admin:**
- `get_limits`, `get_data_kit_deployment_status`

### ❌ Known Issues (88)

| Category | Count | Notes |
|----------|-------|-------|
| Expected 404s | ~35 | Tools work, but need real test data in org |
| Server 500 errors | ~10 | Salesforce backend bugs |
| API field mismatches | ~25 | CREATE operations have undocumented schemas |
| Features not enabled | ~8 | Requires specific org configuration |
| Other validation | ~10 | Input format issues |

**Server-side bugs (cannot fix):**
- `search_tables`, `count_segment`, `get_profile_metadata` - Internal server errors
- `create_data_model_object`, `update_data_model_object`, `delete_data_model_object` - 500 errors
- `create_identity_ruleset` - Backend has `${suffix}` placeholder bug

**Feature not available:**
- `list_private_network_routes` - Requires private network feature
- `run_data_stream` - Endpoint returns 404
- `get_data_kit_status` - Requires data kit feature

**CREATE operations with undocumented APIs:**
Most CREATE/UPDATE operations fail due to undocumented field requirements. The GET/LIST operations work correctly.
